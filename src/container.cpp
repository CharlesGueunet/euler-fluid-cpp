...
void Container::Step() {
  ...
  this->physics.Diffuse(0, this->previousDensity, this->density, this->diff,
                        this->dt, 16, this->size);
  this->physics.Advect(0, this->density, this->previousDensity, this->x,
...
float Container::MapToRange(float val, float minIn, float maxIn, float minOut,
                            float maxOut) {
  ...
  for (int i = 0; i < this->size; i++) {
    for (int j = 0; j < this->size; j++) {
      ...
      case Color::Hsb:
        ...
        break;
      case Color::Velocity: {
        int r = (int)this->MapToRange(this->x[IX(i, j, this->size)], -0.05f,
                                      0.05f, 0, 255);
        int g = (int)this->MapToRange(this->y[IX(i, j, this->size)], -0.05f,
                                      0.05f, 0, 255);
